...
Description
This plugin adds a "Daily" column to the PowerTeacher "Record Meeting Attendance" screen which displays the current (non-Null) daily attendance code for the students.  Even if teachers are able to set Meeting attendance for previous days, the Daily Code will still reflect the attendance on the current calendar day (not previous).
